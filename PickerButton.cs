    ...
    public class PickerButton : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private void SetBackgroundSprite(bool category)
        {
            if (background != null)
            {
                ...
                background.sprite = ImageUtilsCommon.Load9SliceSprite(Plugin.GetAssetPath(spriteName), new RectOffset(Slice, Slice, Slice, Slice));
            }
        }
        ...
        public static PickerButton Create(Transform parent, TextMeshProUGUI textPrefab, Action<AutosorterFilter> action, int width = 100, int height = 18)
        {
            ...
            button.background.sprite = ImageUtilsCommon.Load9SliceSprite(Plugin.GetAssetPath("MainMenuStandardSprite.png"), new RectOffset(Slice, Slice, Slice, Slice));
