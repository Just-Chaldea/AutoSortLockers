...
namespace AutosortLockers
{
    class CustomizeScreen : MonoBehaviour
    {
        ...
        internal void Initialize(SaveDataEntry saveData)
        {
            ...
            exitButton.GetComponentInChildren<Image>().sprite = ImageUtilsCommon.LoadSprite(Plugin.GetAssetPath("Close.png"));
        ...
        {
            target.LabelColor = Plugin.colors[index];
            onModified();
            colorPicker.onSelect -= OnLabelColorPicked;
            SetColor(target);
        }
        ...
        {
            target.IconColor = Plugin.colors[index];
            onModified();
            colorPicker.onSelect -= OnIconColorPicked;
            SetColor(target);
        }
        ...
        {
            target.OtherTextColor = Plugin.colors[index];
            onModified();
            colorPicker.onSelect -= OnTextColorPicked;
            SetColor(target);
        }
        ...
        {
            target.ButtonsColor = Plugin.colors[index];
            onModified();
            colorPicker.onSelect -= OnButtonsColorPicked;
            SetColor(target);
        }
        ...
        {
            target.LockerColor = Plugin.colors[index];
            onModified();
            colorPicker.onSelect -= OnLockerColorPicked;
            SetColor(target);
        }
        ...
        public static CustomizeScreen Create(Transform parent, SaveDataEntry data)
        {
            ...
            screen.background.sprite = ImageUtilsCommon.LoadSprite(Plugin.GetAssetPath("CustomizeScreen.png"));
            ...
            screen.label = LabelController.Create(data, screen.background.transform, textPrefab);
            ...
            var startX = 0;
            var startY = 30;
            screen.labelColorSetting = ColorSetting.Create(screen.background.transform, textPrefab, "Label Color");
            screen.labelColorSetting.rectTransform.anchoredPosition = new Vector2(startX, startY);

            screen.iconColorSetting = ColorSetting.Create(screen.background.transform, textPrefab, "Icon Color");
            screen.iconColorSetting.rectTransform.anchoredPosition = new Vector2(startX, startY - 19);

            screen.textColorSetting = ColorSetting.Create(screen.background.transform, textPrefab, "Filters Color");
            screen.textColorSetting.rectTransform.anchoredPosition = new Vector2(startX, startY - (19 * 2));

            screen.buttonsColorSetting = ColorSetting.Create(screen.background.transform, textPrefab, "Misc Color");
            screen.buttonsColorSetting.rectTransform.anchoredPosition = new Vector2(startX, startY - (19 * 3));

            screen.lockerColorSetting = ColorSetting.Create(screen.background.transform, textPrefab, "Locker Color");
            screen.lockerColorSetting.rectTransform.anchoredPosition = new Vector2(startX, startY - (19 * 4));

            screen.colorPicker = ColorPicker.Create(screen.background.transform, textPrefab);
